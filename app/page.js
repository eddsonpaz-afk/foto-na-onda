...
export default function Home() {
  ...
  async function openCamera(mode = facingMode) {
    ...
    try {
      ...
      setCameraError(
        "Não consegui abrir a câmera. Toque no cadeado do navegador e permita o acesso."
      );
    }
  }
  ...
  function switchCamera() {
    ...
    setZoom(1);
    setOffset({ x: 0, y: 0 });
    window.setTimeout(() => openCamera(next), 50);
  }
            ...
            >
              ...
              {cameraError && (
                <div className="camera-error">
                  <p>{cameraError}</p>
                  <button onClick={() => openCamera(facingMode)}>TENTAR NOVAMENTE</button>
                </div>
              )}
            </div>
