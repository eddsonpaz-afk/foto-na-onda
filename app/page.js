...
export default function Home() {
  ...
  async function openCamera(options = {}) {
    ...
    try {
      ...
      const isFront =
        /front|user|facetime|frontal/.test(label) ||
      ...
      console.error(error);
      setCameraError(
        "Não consegui abrir a câmera traseira. Toque no cadeado do navegador, permita o acesso e tente novamente."
      );
    }
  }
  ...
  async function switchCamera() {
    ...
    await openCamera({ facingMode: nextFacingMode });
  }
            ...
            >
              ...
              {cameraError && (
                <div className="camera-error">
                  <p>{cameraError}</p>
                  <button onClick={() => openCamera({ facingMode: "environment" })}>TENTAR NOVAMENTE</button>
                </div>
              )}
            </div>
